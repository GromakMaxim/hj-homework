class Autocomplete {
  ...
  renderMatches( matches ) {
    ...
    getMatches(text) {
        let autocompleteWords = this.input;
        let resultArr = new Array();
        let len = text.length;

        for (let word of autocompleteWords) {
            let inputPart = text.substring(0, len);
            let wordPart = word.textContent.substring(0, len);

            if (inputPart.match(wordPart)) {
                resultArr.push(
                    {
